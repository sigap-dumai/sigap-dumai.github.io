...
async function fetchWithCache(url, cacheKey, ttlMs, parser = 'json') {
    ...
    try {
        ...
        if (cachedRaw) {
            ...
            if (cached && cached.ts && (now - cached.ts) <= ttlMs && cached.data) {
                // Serve cache cepat jika masih fresh
    ...
    try {
        ...
        // Fallback: serve cache (stale) jika ada
        try {
            const cachedRaw = localStorage.getItem(cacheKey);
            if (cachedRaw) {
                const cached = JSON.parse(cachedRaw);
    ...
    btn.addEventListener('click', () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (pos) => {
                ...
                const nearReports = checkProximity(userLat, userLng, 1.0); // Cek radius 1.0 km
                ...
                if (nearReports.length > 0) {
                    ...
                    nearReports.forEach(r => {
                        message += `- [${r.type}] ${r.name} (${r.distance.toFixed(2)} km)\n`;
                    });
                    alert(message);
                } else {
...
// Fungsi Cuaca, Gempa, Berita, Status
